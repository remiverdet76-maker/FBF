...
function setMasterFreq(f) {
  masterFreq = Math.max(36, Math.min(648, f));
  updateDisplay();
...
function masterStep(delta) {
  setMasterFreq(Math.max(36, Math.min(648, masterFreq + delta)));
}
function onMasterInput(raw) {
  ...
  PAIRS.forEach((pair, i) => {
    const pF = Math.max(36, Math.min(648, v * RATIO_OPTS[pair.pingala.ri].r * pair.pingala.n));
    const iF = Math.max(36, Math.min(648, pF + pair.ida.polarity * pair.ida.delta));
...
function onMasterChange(raw) {
  const v = Math.max(36, Math.min(648, parseInt(raw)));
