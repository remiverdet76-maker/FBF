...
function getProgRange() {
  const lo = Math.max(36,   Math.min(648, parseInt(document.getElementById('prog-min')?.value)||36));
  const hi = Math.max(lo+1, Math.min(648, parseInt(document.getElementById('prog-max')?.value)||648));
  return { lo, hi };
}
function progStep() {
...
function loadState() {
  try {
    ...
    if (!s) return;
    if (s.masterFreq>=36&&s.masterFreq<=648) masterFreq = s.masterFreq;
...
function loadPreset(slot) {
  ...
  if (p.masterFreq>=36 && p.masterFreq<=648) setMasterFreq(p.masterFreq);
